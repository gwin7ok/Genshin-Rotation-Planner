...
const WEAPON_TYPES_LIST: Array<{ id: WeaponType | 'all'; label: string; icon: string }> = [
...
const ELEMENT_TYPES_LIST: Array<{ id: ElementType | 'all'; label: string; color: string; icon: string }> = [
...
export const PartyConfigModal: React.FC<PartyConfigModalProps> = ({
  ...
  const filteredRoster = database.characters.filter(rosterChar => {
    const matchesElement = elementFilter === 'all' || rosterChar.element === elementFilter;
    const matchesWeapon = weaponFilter === 'all' || rosterChar.weaponType === weaponFilter;
    ...
    return matchesElement && matchesWeapon && matchesSearch;
  });
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 bg-slate-950/70 p-2.5 rounded-xl border border-slate-800/80">
              ...
              <div className="flex flex-wrap items-center gap-1">
                ...
                {ELEMENT_TYPES_LIST.map((elem) => {
                  const isActive = elementFilter === elem.id;
                  return (
                    <button
                      key={elem.id}
                      onClick={() => setElementFilter(elem.id)}
              ...
              <div className="flex flex-wrap items-center gap-1 pt-1.5 border-t border-slate-800/60">
                ...
                {WEAPON_TYPES_LIST.map((w) => {
                  const isActive = weaponFilter === w.id;
                  return (
                    <button
                      key={w.id}
                      onClick={() => setWeaponFilter(w.id)}
                      className={`flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-bold transition-all border ${
                        isActive
